...
let lighting = menuData.find((item) => item.handle === "lighting")

let Lamps = lighting.categories.find((item) => item.handle === 'Lamps')
let candleLights = lighting.categories.find((item) => item.handle === 'candle-lights')
        ...
        <div className='w-[90%] mx-auto grid grid-cols-3 border border-[#d6d6d6]'>
          <div className='px-5 flex flex-col gap-4 h-full pt-7 border-s border-[#d6d6d6]'>
            <div className='h-[200px]'>
              <div>
                <Link href={`/shop/${lighting.handle}`} className=" relative block pb-2 border-b border-gray-300 after:absolute after:left-0 after:bottom-px 
                  ...
                  <h3 className=' pb-2'>{lighting.title}</h3>
                </Link>
              </div>
              <div className='pt-5 flex flex-col gap-4'>
                {
                  Lamps.items.map((item, idx) => {
                    return (
          ...
          <div className='px-5 flex flex-col gap-4 h-full pt-7 border-s border-[#d6d6d6]'>
            <div className='h-[200px]'>
              <div>
                <Link href={`/shop/${candleLights.handle}`} className=" relative block pb-2 border-b border-gray-300 after:absolute after:left-0 after:bottom-px 
                             after:h-px  after:w-full after:bg-[#9F1D27] after:origin-right after:scale-x-0 after:transition-transform after:duration-300 hover:after:origin-left
                             hover:after:scale-x-100 hover:text-[#9F1D27]">
                  <h3 className=' pb-2'>{candleLights.title}</h3>
                </Link>
              </div>
              <div className='pt-5 flex flex-col gap-4'>
                {
                  candleLights.items.map((item, idx) => {
                    return (
                      <Link key={idx} href={`/shop/${item.handle}`} className="
                                        relative
                                        w-fit
